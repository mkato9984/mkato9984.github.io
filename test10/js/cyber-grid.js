// マトリックス風の格子状背景を生成するファイル
const gridConfig = {
  // 色の設定（マトリックススタイルの緑系カラー - 透明度を下げる）
  baseColor: 'rgba(0, 255, 0, 0.6)', // マトリックスグリーン（透明度を下げる）
  accentColor: 'rgba(51, 255, 51, 0.6)', // 明るいグリーン（透明度を下げる）
  dreamColor1: 'rgba(0, 170, 0, 0.6)', // 暗めのグリーン（透明度を下げる）
  dreamColor2: 'rgba(0, 51, 0, 0.6)', // さらに暗いグリーン（透明度を下げる）
  dreamColor3: 'rgba(102, 255, 102, 0.6)', // 明るいグリーン（透明度を下げる）
  glowColor: 'rgba(0, 255, 0, 0.4)', // グリーンの発光（透明度を下げる）
  pulseColor: 'rgba(0, 200, 0, 0.3)', // 暗めのグリーンのパルス（透明度を下げる）
  energyColor: 'rgba(120, 255, 120, 0.4)', // 明るいグリーンのエネルギー（透明度を下げる）
  
  // 格子の設定 - サイズを少し小さく
  gridSize: 1800, // グリッドサイズを小さく
  gridDivisions: 14, // 分割数を少し減らす
  gridLayers: 2, // 層の数を減らす
  lineWidth: 1.0, // 線を細くする
  perspective: 700, // パースペクティブを調整
  
  // 点の設定 - 数を減らす
  pointCount: 80, // デジタルレインのような点を減らす
  pointSize: 1.5, // サイズを小さく
  pointMinSize: 0.8,
  pointMaxSize: 2,
  
  // アニメーション設定
  rotationSpeedX: 0, // 基本は停止
  rotationSpeedY: 0,
  rotationSpeedZ: 0,
  pulseFrequency: 0.4, // パルスの頻度を下げる
  energyWaveSpeed: 0.15, // エネルギー波の速度を下げる
  
  // エフェクト設定 - 全体的に弱める
  glowStrength: 1.5, // 線の発光を弱める
  lineGlowSize: 2.0, // 線の発光サイズを小さく
  connectionMaxDistance: 80, // 接続線の距離を短く
  connectionOpacity: 0.2, // 接続線の透明度を上げる
  mouseInfluence: 200, // マウスの影響範囲を小さく
  mouseStrength: 0.1, // マウスの影響の強さを弱める
  
  // ランダム発光の設定
  randomGlowChance: 0.5, // 確率を下げる
  randomGlowColors: [
    'rgba(255, 255, 255, 0.5)',  // 白（透明度を下げる）
    'rgba(0, 255, 0, 0.4)',      // マトリックスグリーン（透明度を下げる）
    'rgba(120, 255, 120, 0.3)',  // 明るいグリーン（透明度を下げる）
    'rgba(0, 180, 0, 0.4)',      // 暗めのグリーン（透明度を下げる）
    'rgba(0, 100, 0, 0.3)',      // さらに暗いグリーン（透明度を下げる）
  ],
  
  // 最適化設定
  fpsLimit: 20, // FPSを少し下げる（負荷軽減）
  cullingDistance: 4500, // 描画距離を短縮
  maxConnections: 50, // 接続数を減らす
  ...
  useFlowEffect: true,
  useMouseEffect: true,
  useDataStreamEffect: true,
  useEnergyWaveEffect: true,
  ...
  // マトリックス風の新しいエフェクト設定 - 頻度を下げる
  dataPaths: 8,
  dataSpeed: 0.5,
  energyNodes: 6,
  gridFlashInterval: 7000, // フラッシュ間隔を長く
  gridFlashDuration: 400, // フラッシュ時間を短く
  
  // マウス効果設定 - 弱める
  mouseTiltStrength: 0.08,
  mouseTiltDecay: 0.99,
  mouseTiltMax: 0.1,
  mouseTiltEasing: true,
  
  // 慣性関連の設定
  useInertia: true,
  inertiaStrength: 0.95,
  inertiaThreshold: 0.002,
  
  // マウス処理の最適化
  mouseSamplingInterval: 100, // サンプリング間隔を長く
  
  // 描画最適化
  simplifyFarEdges: true,
  distanceOptimization: true,
  skipFrames: 2, // スキップフレーム数を増やす
  
  // マトリックス特有の設定
  matrixDigitalRain: true,
  rainDropSpeed: 1.0, // 速度を下げる
...
class CyberGrid {
  ...
  drawMysticalEffects() {
    ...
    this.ctx.strokeStyle = `rgba(0, 220, 0, ${0.08 + pulse * 0.04})`;
    ...
    for (let i = 0; i < patternCount; i++) {
      ...
      this.ctx.strokeStyle = `rgba(0, 230, 0, ${0.1 + pulse * 0.05})`;
      ...
      this.ctx.strokeStyle = `rgba(0, 200, 0, ${0.07 + pulse * 0.03})`;
      this.ctx.lineWidth = 0.5;
      this.ctx.stroke();
    }
    ...
    coreGradient.addColorStop(0.5, 'rgba(0, 220, 0, 0.3)');
    coreGradient.addColorStop(1, 'rgba(0, 180, 0, 0)');
...
window.initCyberGridBackground = function() {
  console.log('マトリックス風アニメーション初期化開始');
  ...
  canvas.style.opacity = '0.7'; // キャンバス全体の透明度を設定
  canvas.style.position = 'absolute';
  canvas.style.top = '0';
  canvas.style.left = '0';
  canvas.style.width = '100%';
  canvas.style.height = '100%';
  canvas.style.zIndex = '1'; // z-indexを1に設定（最背面）
  heroSection.prepend(canvas);
  
  // グラデーントオーバーレイを作成（マトリックス風のグラデーション）
  gradientOverlay = document.createElement('div');
  gradientOverlay.classList.add('gradient-overlay');
  gradientOverlay.style.background = 'radial-gradient(circle at center, rgba(0, 200, 0, 0.03) 0%, rgba(0, 150, 0, 0.02) 40%, rgba(0, 100, 0, 0.01) 70%, rgba(0, 50, 0, 0.005) 100%)';
  gradientOverlay.style.position = 'absolute';
  gradientOverlay.style.top = '0';
  gradientOverlay.style.left = '0';
  gradientOverlay.style.width = '100%';
  gradientOverlay.style.height = '100%';
  gradientOverlay.style.zIndex = '1'; // z-indexを1に設定（サイバーグリッドと同じ層）
  ...
  // キャッチコピーとボタンのスタイルを強化して必ず前面に表示されるようにする
  ...
  // テキスト要素のスタイルを強化
  const elements = [
    { selector: '.hero__title', zIndex: 11 },
    { selector: '.hero__subtitle', zIndex: 11 },
    { selector: '.condensed-text', zIndex: 11 },
    { selector: '.hero__buttons', zIndex: 11 },
    { selector: '.btn--primary', zIndex: 11 },
    { selector: '.btn--secondary', zIndex: 11 },
    { selector: '.cyber-corner', zIndex: 10 },
    { selector: '.scroll-indicator', zIndex: 11 }
  ];
  ...
  elements.forEach(item => {
    ...
    elements.forEach(el => {
      ...
      el.style.zIndex = item.zIndex.toString();
    });
  });
  ...
  console.log('マトリックス風アニメーション初期化完了');
};

document.addEventListener('DOMContentLoaded', function() {
  console.log('DOMContentLoaded: マトリックス風アニメーション初期化を実行します');
