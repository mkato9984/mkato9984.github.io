...
window.initDotRippleBackground = function() {
  ...
  let dotConfig = {
    ...
    minBaseSize: 1.2,
    autoActivate: false,
    activateOnMouseMove: true,
    activateOnClick: true,
    mouseSensitivity: 50,
    gridSnap: false,
    useColorCycle: true,
    useInterference: true,    // 干渉パターンを有効化
    useGlowEffect: true,      // 発光エフェクトを有効化
    useParticles: true,       // 幻想的な粒子エフェクトを有効化
    maxParticles: 60,         // 最大粒子数
    blendMode: 'lighter',     // 描画ブレンドモード
    startupDelay: 1000,       // 起動時のディレイ(ms)
  };
  ...
  function createRipple(x, y, strength, isNatural = false, colorType = 'normal') {
    ...
    switch (colorType) {
      case 'accent':
        rippleBaseColor = dotConfig.baseColor;
        rippleTargetColor = dotConfig.accentColor;
        break;
      case 'dream1':
        rippleBaseColor = dotConfig.baseColor;
        rippleTargetColor = dotConfig.dreamColor1;
        break;
      case 'dream2':
        rippleBaseColor = dotConfig.rippleColor;
        rippleTargetColor = dotConfig.dreamColor2;
        break;
      case 'dream3':
        rippleBaseColor = dotConfig.accentColor;
        rippleTargetColor = dotConfig.dreamColor3;
        break;
      case 'cyclic':
        rippleBaseColor = getCyclicColor(0.5, colorCycle, 1.0);
        rippleTargetColor = getCyclicColor(0.2, colorCycle + 0.3, 1.0);
        break;
      default:
        rippleBaseColor = dotConfig.baseColor;
        rippleTargetColor = dotConfig.rippleColor;
    }
  ...
  function updateMouseSpeed(x, y) {
    ...
    if (lastMouseX > -999 && lastMouseY > -999) {
      ...
      if (timeDiff < 100 && timeDiff > 0) {
        mouseSpeed = distance / Math.max(1, timeDiff * 0.03);
        mouseSpeed = Math.min(mouseSpeed, 8);
      }
    }
    ...
    if (mouseSpeed > 1.5) {
      const strengthMultiplier = Math.min(1.0, mouseSpeed / 4);
      const colorType = mouseSpeed > 5 ? 'dream3' : (mouseSpeed > 3 ? 'dream1' : 'normal');
      createRipple(x, y, strengthMultiplier * 0.6, false, colorType);
    }
  }
  ...
  function drawDots() {
    ...
    if (dotConfig.useInterference) {
      ...
      ripples.forEach(ripple => {
        ...
        const primaryColor = ripple.baseColor;
        const secondaryColor = ripple.targetColor;
        
        waveGradient.addColorStop(0, `rgba(255, 255, 255, ${ripple.opacity * 0.1})`);
        waveGradient.addColorStop(0.4, `rgba(${primaryColor.match(/\d+/g)?.[0] || 100}, ${primaryColor.match(/\d+/g)?.[1] || 150}, ${primaryColor.match(/\d+/g)?.[2] || 255}, ${ripple.opacity * 0.05})`);
        waveGradient.addColorStop(0.7, `rgba(${secondaryColor.match(/\d+/g)?.[0] || 50}, ${secondaryColor.match(/\d+/g)?.[1] || 100}, ${secondaryColor.match(/\d+/g)?.[2] || 200}, ${ripple.opacity * 0.02})`);
        ...
        if (ripple.pattern === 2 || ripple.pattern === 3) {
          ctx.strokeStyle = `rgba(255, 255, 255, ${ripple.opacity * 0.15})`;
          ctx.lineWidth = 1;
