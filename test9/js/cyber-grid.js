...
const gridConfig = {
  // 色の設定（サイバーパンクスタイルのネオンカラー）
  baseColor: '#00FFFF', // シアン
  accentColor: '#FF00FF', // マゼンタ
  dreamColor1: '#00FF00', // ネオングリーン
  dreamColor2: '#FFFF00', // イエロー
  dreamColor3: '#FF00FF', // マゼンタ
  glowColor: 'rgba(0, 255, 255, 0.8)', // シアンの強い発光
  pulseColor: 'rgba(255, 0, 255, 0.5)', // マゼンタのパルス
  energyColor: 'rgba(0, 255, 0, 0.8)', // グリーンの強いエネルギー
  
  // 格子の設定
  gridSize: 2000, // より大きなグリッドサイズに
  gridDivisions: 14, // 分割数を増やして細かく
  gridLayers: 4, // 層の数を増加
  lineWidth: 1.8, // 線を少し太く
  ...
  // エフェクト設定
  glowStrength: 2.5, // 線の発光をかなり強く
  lineGlowSize: 3.5, // 線の発光サイズを大きく
  ...
  // ランダム発光の設定
  randomGlowChance: 0.5, // 各フレームでランダム発光する確率を高める
  randomGlowColors: [
    'rgba(255, 255, 255, 0.9)',  // 白（強い）
    'rgba(0, 255, 255, 0.8)',    // シアン（強い）
    'rgba(255, 0, 255, 0.8)',    // マゼンタ（強い）
    'rgba(0, 255, 0, 0.8)',      // グリーン（強い）
    'rgba(255, 255, 0, 0.8)',    // イエロー（強い）
  ],
  
  // 最適化設定
  fpsLimit: 20, // FPSをさらに低く制限
  cullingDistance: 5000, // 描画距離を少し縮小して負荷軽減
  maxConnections: 60, // 接続数を減らす
  
  // エフェクト有効/無効
  usePulseEffect: true, // パルスエフェクトを有効化
  useGlowEffect: true,  // グローエフェクトを有効化
  useFlowEffect: false,
  useMouseEffect: true, // マウスの効果を有効化
  useDataStreamEffect: false,
  useEnergyWaveEffect: true, // エネルギー波効果を有効化
  ...
  dataSpeed: 0.4,
  energyNodes: 6,
  gridFlashInterval: 3000, // グリッドがフラッシュする間隔を短く（派手に）
  gridFlashDuration: 800, // フラッシュの持続時間を長めに
  
  // マウス効果設定
  mouseTiltStrength: 0.15, // 傾きの強さを微調整
  mouseTiltDecay: 0.992,
  mouseTiltMax: 0.15,
  mouseTiltEasing: true,
  
  // 慣性関連の設定
  useInertia: true,
  inertiaStrength: 0.97,
  inertiaThreshold: 0.001,
  
  // マウス処理の最適化
  mouseSamplingInterval: 80,
